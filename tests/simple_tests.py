...
def test_single_shard_matrix():
    ...
    X = data.data
    X_sharded = Matrix("test", shape=X.shape, shard_sizes=X.shape, bucket="lambdapackpublic")
...
def test_multiple_shard_matrix():
    data = datasets.load_digits()
    X = data.data
    X_sharded = Matrix("test", shape=X.shape, shard_sizes=[128,32], bucket="lambdapackpublic")
    X_sharded.shard_matrix(X)
